<div>
    <x-status />
    <div class="mb-5 text-2xl font-bold">DEPARTEMENTS</div>
    <div class="mt-8">
    ...
    <div class="max-w-2xl overflow-hidden rounded-lg border border-gray-200 shadow-md">
        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-400">Nom de la Departement</th>
            ...
            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                @foreach ($departement as $departements)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 font-medium text-gray-900">{{ $departements->nom_departement }}</td>
                        <td class="flex justify-end gap-4 px-6 py-4 font-medium">
                            <a href="{{ route('departement.edit', $departements) }}"
                                class="rounded-full bg-blue-400 px-4 py-1 text-blue-800 hover:bg-blue-500 hover:text-white">Edit</a>
                            <button wire:confirm="Are you sure you want to delete this Departement?"
                                wire:click="deleteDepartement({{ $departements->id }})"
        ...
        <div class="bg-white p-4">{{ $departement->links() }}</div>
    </div>
